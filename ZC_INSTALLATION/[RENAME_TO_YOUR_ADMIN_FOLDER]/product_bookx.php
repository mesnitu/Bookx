 ...
 * @version $Id: [admin]/product_bookx.php 2016-02-02 philou $
 */
...
//die(DIR_WS_MODULES . $zc_products->get_handler($product_type) . '/copy_to_confirm.php');
      ...
      case 'new_product_preview':
        if (file_exists(DIR_WS_MODULES . $zc_products->get_handler($product_type) . '/new_product_preview.php')) {
         ...
         //** look for additional new_product_preview_*.php files and include now **//
         $incl_dir = @dir(DIR_FS_ADMIN . '/includes/modules/product_bookx');
         while ($file = $incl_dir->read()) {
         	if ('new_product_preview_' == substr($file, 0, 20)) {
         		include_once DIR_FS_ADMIN . '/includes/modules/product_bookx/' . $file; // This should handle any extra POST'ed files etc. collected in collect_info*.php
         	}
         }
         $incl_dir->close();
        break;
