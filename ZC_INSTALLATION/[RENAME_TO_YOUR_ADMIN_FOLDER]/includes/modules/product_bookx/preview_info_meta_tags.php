...
if(zen_not_null($_POST)) {
    $pInfoMetatags = new objectInfo($_POST);
    ...
    $pInfoMetatags = new objectInfo($product->fields);
}
...
<div class="container-fluid">
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
        $pInfoMetatags->metatags_title = zen_db_prepare_input($metatags_title[$languages[$i]['id']]);
        $pInfoMetatags->metatags_keywords = zen_db_prepare_input($metatags_keywords[$languages[$i]['id']]);
        $pInfoMetatags->metatags_description = zen_db_prepare_input($metatags_description[$languages[$i]['id']]);
        ?>

        <table class="table table-bordered">
            <tr>
                <td class="main" valign="top">
                    <?php echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . '<strong>' . TEXT_PRODUCTS_NAME . '</strong>' . '&nbsp;' . ($pInfoMetatags->metatags_products_name_status == '1' ? zen_get_products_name($_GET['pID'], $languages[$i]['id']) : TEXT_META_EXCLUDED); ?>
                </td>
                <td class="main" valign="top">
                    <?php echo '<strong>' . TEXT_PRODUCTS_MODEL . '</strong>&nbsp;' . ($pInfoMetatags->metatags_model_status == '1' ? $pInfoMetatags->products_model : TEXT_META_EXCLUDED); ?>
                </td>
                <td class="main" valign="top">
                    <?php echo '<strong>' . TEXT_PRODUCTS_PRICE_INFO . '</strong>&nbsp;' . ($pInfoMetatags->metatags_price_status == '1' ? $currencies->format($pInfoMetatags->products_price_sorter) : TEXT_META_EXCLUDED); ?>
                </td>
                <td class="main" valign="top">
                    <?php echo '<strong>' . TEXT_PRODUCTS_METATAGS_TITLE_TAGLINE_STATUS . '</strong>&nbsp;' . ($pInfoMetatags->metatags_title_tagline_status == '1' ? TITLE . ' ' . SITE_TAGLINE : TEXT_META_EXCLUDED); ?>
                </td>
            </tr>
            <tr>
                <td class="main" valign="top"><?php echo TEXT_META_TAGS_TITLE; ?>&nbsp;</td>
                <td class="main" colspan="3"><?php echo ($pInfoMetatags->metatags_title_status == '1' ? $pInfoMetatags->metatags_title : TEXT_META_EXCLUDED); ?></td>
            </tr>
            <tr>
                <td class="main" valign="top"><?php echo TEXT_META_TAGS_KEYWORDS; ?>&nbsp;</td>
                <td class="main" colspan="3"><?php echo $pInfoMetatags->metatags_keywords; ?></td>
            </tr>
            <tr>
                <td class="main" valign="top"><?php echo TEXT_META_TAGS_DESCRIPTION; ?>&nbsp;</td>
                <td class="main" colspan="3"><?php echo $pInfoMetatags->metatags_description; ?></td>
            </tr>
        </table>
        <?php
    }
    ...
    <div class="row text-right">
        ...
        if($preview == false) {
            ...
            <input type="button" name="edit" class="btn btn-default" value="<?php echo IMAGE_BACK; ?>"> <button type="submit" class="btn btn-primary"><?php echo (isset($_GET['pID']) ? IMAGE_UPDATE : IMAGE_INSERT); ?></button> <a href="<?php echo zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . (isset($_GET['pID']) ? '&pID=' . $_GET['pID'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')); ?>" class="btn btn-default" role="button"><?php echo IMAGE_CANCEL; ?></a>
        </div>
<?php } ?>
</form>
</div>
