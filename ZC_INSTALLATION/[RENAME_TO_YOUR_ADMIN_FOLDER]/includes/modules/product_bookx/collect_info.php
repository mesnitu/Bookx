...
if (isset($_GET['pID']) && empty($_POST)) { //" . DATE_FORMAT_SHORT . "
    ...
    $sql = 'SELECT pd.products_name, pd.products_description, pd.products_url, 
        ...
        LEFT JOIN ' . TABLE_PRODUCT_BOOKX_EXTRA_DESCRIPTION . ' bed ON be.products_id = bed.products_id AND bed.languages_id = "' . (int)$_SESSION['languages_id'] . '"
    ...
    while (!$assigned_authors->EOF) {
        $product_assigned_authors[] = array(
            'primary_id' => $assigned_authors->fields['primary_id'], 
    ...
    while (!$assigned_genres->EOF) {
        $product_assigned_genres[] = array(
            'primary_id' => $assigned_genres->fields['primary_id'], 
...
$author_types = $db->Execute('SELECT at.bookx_author_type_id, atd.type_description FROM ' . TABLE_PRODUCT_BOOKX_AUTHOR_TYPES . ' at 
    LEFT JOIN ' . TABLE_PRODUCT_BOOKX_AUTHOR_TYPES_DESCRIPTION . ' atd ON 
    at.bookx_author_type_id = atd.bookx_author_type_id AND atd.languages_id = "' . $_SESSION['languages_id'] . '"
...
$publisher = $db->Execute("SELECT bookx_publisher_id, publisher_name FROM " . TABLE_PRODUCT_BOOKX_PUBLISHERS . " ORDER BY publisher_name ASC");
while (!$publisher->EOF) {
    $publisher_array[] = array(
        'id' => $publisher->fields['bookx_publisher_id'],
...
switch ($pInfo->products_priced_by_attribute) {
    case '0': $is_products_priced_by_attribute = false;
        $not_products_priced_by_attribute = true;
        break;
    case '1': $is_products_priced_by_attribute = true;
        $not_products_priced_by_attribute = false;
        break;
    default: $is_products_priced_by_attribute = false;
        $not_products_priced_by_attribute = true;
}
// Product is Free
