...
// Copy attributes to duplicate product
  ...
  if ($_POST['copy_as'] == 'link') {
    ...
    $bookx_extra = $db->Execute("SELECT *
                                 FROM " . TABLE_PRODUCT_BOOKX_EXTRA . "
                                 WHERE products_id = " . (int)$products_id);

//            var_dump($bookx_extra);
//            die();
    $db->Execute("INSERT INTO " . TABLE_PRODUCT_BOOKX_EXTRA . " (products_id, bookx_publisher_id, bookx_series_id, bookx_imprint_id, bookx_binding_id, bookx_printing_id, bookx_condition_id, publishing_date, pages, volume, size, isbn)
                  VALUES (" . (int)$dup_products_id . ",
    ...
    $bookx_assigned_genres = $db->Execute("SELECT bookx_genre_id
                                           FROM " . TABLE_PRODUCT_BOOKX_GENRES_TO_PRODUCTS . "
                                           WHERE products_id = " . (int)$products_id);

    $db->Execute("INSERT INTO " . TABLE_PRODUCT_BOOKX_GENRES_TO_PRODUCTS . " (products_id, bookx_genre_id)
                  VALUES (" . (int)$dup_products_id . ", '" . (int)$bookx_assigned_genres->fields['bookx_genre_id'] . "')");

    $bookx_assigned_authors = $db->Execute("SELECT bookx_author_id, bookx_author_type_id
                                            FROM " . TABLE_PRODUCT_BOOKX_AUTHORS_TO_PRODUCTS . "
                                            WHERE products_id = " . (int)$products_id);

    $db->Execute("INSERT INTO " . TABLE_PRODUCT_BOOKX_AUTHORS_TO_PRODUCTS . " (products_id, bookx_author_id, bookx_author_type_id)
                  VALUES (" . (int)$dup_products_id . ", " . (int)$bookx_assigned_authors->fields['bookx_author_id'] . ", " . (int)$bookx_assigned_authors->fields['bookx_author_type_id'] . ")");
