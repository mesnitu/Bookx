...
<?php echo tpl_bookx_alphafilter_all($index, FILENAME_BOOKX_IMPRINTS_LIST); ?>

<div id="bookxImprintListingTable" class="bookxFilterListAll">

<?php
	foreach ($bookx_imprints_listing_split_array as $imprint) {
		echo '<div class="row">' . zen_image($imprint['imprint_image'], $imprint['imprint_name'], BOOKX_IMPRINT_LISTING_IMAGE_MAX_WIDTH, BOOKX_IMPRINT_LISTING_IMAGE_MAX_HEIGHT, 'class="bookxAllListingImage"');
		echo '<h3 class="bookxAllListingInfo"><span class="bookxImprintName">' . $imprint['imprint_name'] . '</span></h3>' 
		     . (!empty($imprint['imprint_description']) ? '<p class="bookxAllListingDescription">' . $imprint['imprint_description'] . '</p>' : '')
