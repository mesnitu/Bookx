...
if (ALLOW_PRODUCT_BOOKX_FILTER_MULTIPLE) {
    // Testing a way to display input hidden fields dynamically. This way other
    $display_hidden_excluding = function ($active_filters, $exclude_field) {
        array_pop($active_filters);
        $hidden_fields = array();
        foreach ($active_filters as $field_id => $value) {
            if ($active_filters[$field_id]) {
                if ($exclude_field == $field_id) {
                    $hidden_fields[$field_id] = '';
                } else {
                    $hidden_fields[$field_id] = zen_draw_hidden_field('bookx_' . $field_id, $active_filters[$field_id]);
                }
            }
        }
        return implode($hidden_fields);
    };
}
...
if ($show_author_filter) {
    if ($show_author_type_filter) {
        ...
        $content .= $display_hidden_excluding($active_bx_filter_ids, 'author_id'); // array / field to
    ...
    //$content .= $hidden_publisher_id_field . $hidden_imprint_id_field . $hidden_series_id_field . $hidden_genre_id_field;
    $content .= $display_hidden_excluding($active_bx_filter_ids, 'author_id');
...
if ($show_publisher_filter) {
    ...
    $content .= $display_hidden_excluding($active_bx_filter_ids, 'publisher_id');
    $content .= zen_draw_hidden_field('main_page', FILENAME_DEFAULT) . zen_hide_session_id() . zen_draw_hidden_field('typefilter', 'bookx');
...
if ($show_imprint_filter) {
    ...
    $content .= $display_hidden_excluding($active_bx_filter_ids, 'imprint_id');
    $content .= zen_draw_hidden_field('main_page', FILENAME_DEFAULT) . zen_hide_session_id() . zen_draw_hidden_field('typefilter', 'bookx');
...
if ($show_series_filter) {
    ...
    $content .= $display_hidden_excluding($active_bx_filter_ids, 'series_id');
    $content .= zen_draw_hidden_field('main_page', FILENAME_DEFAULT) . zen_hide_session_id() . zen_draw_hidden_field('typefilter', 'bookx');
...
if ($show_genre_filter) {
    ...
    $content .= $display_hidden_excluding($active_bx_filter_ids, 'genre_id');
    $content .= zen_draw_hidden_field('main_page', FILENAME_DEFAULT) . zen_hide_session_id() . zen_draw_hidden_field('typefilter', 'bookx');
