...
if (!empty($bookx_upcoming_products_look_ahead_number_of_days)) {
    ...
    $extra_where_condition = ' OR ((be.publishing_date IS NOT NULL) AND
        (DATEDIFF("' . date('Y-m-d') . '",
        CONCAT_WS("-",
...
$sql = 'SELECT a.*, a.author_url, ad.author_description, at.type_sort_order, atd.type_description, atd.type_image,
        ...
        CASE WHEN relp.products_quantity = 0 AND 
        (relp.products_date_available IS NULL) AND 
        (relbe.publishing_date IS NULL OR relbe.publishing_date <= "' . date('Y-m-d 00:00:00', time()) . '") 
        THEN "out_of_print"
        WHEN relp.products_quantity = 0 AND 
        (relp.products_date_available IS NOT NULL) AND 
        (relbe.publishing_date IS NULL OR relbe.publishing_date < "' . date('Y-m-d 00:00:00', time()) . '") AND
        (relp.products_date_available >= "' . date('Y-m-d 00:00:00', time()) . '") 
        THEN "temporarily_unavailable"
        WHEN (relbe.publishing_date IS NOT NULL) AND 
        relbe.publishing_date >= "' . date('Y-m-d 00:00:00', time()) . '" AND
        relbe.publishing_date <= "' . date('Y-m-d 00:00:00', time() + 86400 * intval($bookx_upcoming_products_look_ahead_number_of_days)) . '"
        THEN "upcoming_product"
        WHEN relp.products_quantity > 0 AND 
        (relbe.publishing_date IS NOT NULL) AND relbe.publishing_date <= "' . date('Y-m-d 00:00:00', time()) . '" AND
...
$sql = 'SELECT g.bookx_genre_id, g.genre_sort_order, gd.genre_name, gd.genre_image FROM ' . TABLE_PRODUCT_BOOKX_GENRES . ' g
            ...
            gtp.bookx_genre_id = g.bookx_genre_id  
...
$sql = 'SELECT *  FROM ' . TABLE_PRODUCT_BOOKX_SERIES . ' s
    		LEFT JOIN ' . TABLE_PRODUCT_BOOKX_SERIES_DESCRIPTION . ' sd ON
            sd.bookx_series_id = s.bookx_series_id AND sd.languages_id = "' . (int) $_SESSION['languages_id'] . '"
...
$sql = 'SELECT *  FROM ' . TABLE_PRODUCT_BOOKX_IMPRINTS . ' i
    		LEFT JOIN ' . TABLE_PRODUCT_BOOKX_IMPRINTS_DESCRIPTION . ' id ON 
            id.bookx_imprint_id = i.bookx_imprint_id AND id.languages_id = "' . (int) $_SESSION['languages_id'] . '"
...
$sql = 'SELECT bd.binding_description  FROM ' . TABLE_PRODUCT_BOOKX_BINDING . ' b
    		LEFT JOIN ' . TABLE_PRODUCT_BOOKX_BINDING_DESCRIPTION . ' bd ON 
            bd.bookx_binding_id = b.bookx_binding_id AND bd.languages_id = "' . (int) $_SESSION['languages_id'] . '"
...
$sql = 'SELECT pd.printing_description  FROM ' . TABLE_PRODUCT_BOOKX_PRINTING . ' p
    		LEFT JOIN ' . TABLE_PRODUCT_BOOKX_PRINTING_DESCRIPTION . ' pd ON 
            pd.bookx_printing_id = p.bookx_printing_id AND pd.languages_id = "' . (int) $_SESSION['languages_id'] . '"
...
$sql = 'SELECT cd.condition_description  FROM ' . TABLE_PRODUCT_BOOKX_CONDITIONS . ' c
    		LEFT JOIN ' . TABLE_PRODUCT_BOOKX_CONDITIONS_DESCRIPTION . ' cd ON 
            cd.bookx_condition_id = c.bookx_condition_id AND cd.languages_id = "' . (int) $_SESSION['languages_id'] . '"
...
while (!$authors->EOF) {
    ...
    $current_author['description_link'] = (!empty($authors->fields['bookx_author_id']) && !empty($authors->fields['author_name']) ? '<a href="' . $link_author_desc . '" class="bookx_searchlink">' . TEXT_BOOKX_MORE_PRODUCT_INFO . '</a>' : '');
    ...
    switch ((int) $flag_show_product_bookx_info_authors_related_products) {
        ...
        case 2:
            if (!empty($authors->fields['related_products'])) {
                ...
                // split concatenated string from query into single product entries
                $temp_relp_array = explode('#§#', $authors->fields['related_products']); 
